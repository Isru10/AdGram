...
export async function POST(
  ...
  try {
    // Await params to extract the chatId
    ...
    // 2. Authorization Check: Is the current user the actual seller for this ad?
    const ad = chat.ad as any; // cast since populate returns a mixed type
    if (ad.seller.toString() !== session.user.id) {
      return NextResponse.json(
        { message: "Forbidden: You are not the owner of this ad." },
        { status: 403 }
      );
    }
    ...
    // Optional: you could add a system message to the chat here
